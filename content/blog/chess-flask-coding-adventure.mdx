Chess Flask started as a backend project I could show on my portfolio website: put a chess bot behind a Flask API, let the site call it, and give people something they could actually [play against in the browser](https://www.sneakyowl.net/chess).
...
The first shape of the repository was easy to explain. [Chess Flask](https://github.com/Shamanbenny/chess-flask) exposes a small Flask backend, accepts a FEN string, and returns a move. The frontend can stay in the [SneakyOwl](https://github.com/Shamanbenny/SneakyOwl) website repository, while the chess engine logic lives in a separate backend repo that is deployed on Vercel.
...
At this point in the project, I was getting rather impatient as I had wanted to learnt about a new automated paradigm for iterative improvement using CLI agents such as Codex, and I knew it was perfect for the project. As a result, the repo moved toward an automated experiment loop through [`autoresearch`](https://github.com/Shamanbenny/chess-flask/tree/main/autoresearch). The idea was appealing: utilize some form of engine-vs-engine evaluation to help approve/reject improvements made by Codex, instead of relying only on manual intuition and implementation.

> I will cover more about this autonomous paradigm in my next blog [here](http://localhost:3000/blog/autoresearch-for-chess-bot).

At first, this decision looked promising. The original [`v2.0`](https://github.com/Shamanbenny/chess-flask/commit/db423caf1b8d3e0f89df051e0ab2777127195a77) passed the [evaluator](https://github.com/Shamanbenny/chess-flask/blob/main/autoresearch/EVALUATE.md) on its [first experiment](https://github.com/Shamanbenny/chess-flask/commit/583348d4340f55faa0855e3035b63b1284b613b1). That proved two useful things. First, the evaluator could approve a real improvement rather than only rejecting changes. Second, the [modified automated workflow](https://github.com/Shamanbenny/chess-flask/tree/main/autoresearch) was not just theoretical; it could actually move the engine forward.

Then the [later attempts](https://github.com/Shamanbenny/chess-flask/blob/8fc86540289d1fb09196b755ddf379a93b45edaa/autoresearch/ATTEMPTS.md) started getting rejected.
...
This is not the end for the project though. From this point on, the focus will shift more towards the `autoresearch` paradigm I had modified to work outside of iterative Neural Network experiments, which I can't wait to share with you more about [here](http://localhost:3000/blog/autoresearch-for-chess-bot)!
